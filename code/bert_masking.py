...
train_dataset = MaskedDataset(train_data + train_data_def, tokenizer,
                              target2idx, mask_size=mask_size)
    ...
    with torch.no_grad():
        with tqdm(total=len(dev_loader)) as pbar:
            for i, (x, y) in enumerate(dev_loader):
                ...
                pbar.update(1)
                result, indices = torch.topk(out, k=100, dim=-1, largest=True,
                                             sorted=True)
...
def getPredFromDesc(model, desc: str, mask_size=5, top_n=10):
    ...
    cls_id, mask_id, sep_id, pad_id = dev_dataset.cls_id, \
        dev_dataset.mask_id, dev_dataset.sep_id, dev_dataset.pad_id
    ...
    out = model(input_ids=x, attention_mask=attention_mask,
                target_matrix=target_matrix)
    result, indices = torch.topk(out, k=top_n, dim=-1, largest=True,
                                 sorted=True)
...
tokenizer.convert_tokens_to_string(
    tokenizer.convert_ids_to_tokens(dev_dataset[120][0]))
