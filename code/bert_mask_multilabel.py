...
train_data, train_data_def, dev_data, \
    test_data_seen, test_data_unseen, test_data_desc = d
mask_size = 5
target_matrix, target2idx, idx2target = make_vocab(d, tokenizer,
                                                   mask_size=mask_size)
model = MaskedRDModel.from_pretrained('bert-base-uncased')
model.initialize(mask_size=mask_size, multilabel=True,
                 ww_vocab_size=len(target2idx), pos_weight=10)
wn_data = read_json('../data/wn_data.json')
wn_categories = ['synonyms', 'hyponyms', 'hypernyms', 'related_forms']
train_dataset = MaskedDataset(train_data + train_data_def,
                              tokenizer,
                              target2idx,
                              wn_data=wn_data,
                              wn_categories=wn_categories,
                              mask_size=mask_size)
dev_dataset = MaskedDataset(dev_data,
                            tokenizer,
                            target2idx,
                            wn_data=wn_data,
                            wn_categories=wn_categories,
                            mask_size=mask_size)
test_dataset_seen = MaskedDataset(test_data_seen,
                                  tokenizer,
                                  target2idx,
                                  wn_data=wn_data,
                                  wn_categories=wn_categories,
                                  mask_size=mask_size)
test_dataset_unseen = MaskedDataset(test_data_unseen,
                                    tokenizer,
                                    target2idx,
                                    wn_data=wn_data,
                                    wn_categories=wn_categories,
                                    mask_size=mask_size)
test_dataset_desc = MaskedDataset(test_data_desc,
                                  tokenizer,
                                  target2idx,
                                  wn_data=wn_data,
                                  wn_categories=wn_categories,
...
train_loader = data.DataLoader(train_dataset,
                               **{'shuffle': True, **loader_params})
dev_loader = data.DataLoader(dev_dataset,
                             **{'shuffle': True, **loader_params})
test_loader_seen = data.DataLoader(test_dataset_seen,
                                   **{'shuffle': False, **loader_params})
test_loader_unseen = data.DataLoader(test_dataset_unseen,
                                     **{'shuffle': False, **loader_params})
test_loader_desc = data.DataLoader(test_dataset_desc,
                                   **{'shuffle': False, **loader_params})
    ...
    with torch.no_grad():
        with tqdm(total=len(loader)) as pbar:
            for i, (x, y, wn_ids) in enumerate(loader):
                ...
                b = len(x)
                acc1, acc10, acc100, pred_rank = evaluate(indices, y,
                                                          test=True)
                test_acc1 += acc1
                test_acc10 += acc10
                test_acc100 += acc100
...
for epoch in range(epoch, epochs + 10):
    ...
    filename = f'../trained_models/{model_name} Epoch {epoch+1} \
        at {datetime.datetime.now()}'.replace(' ', '_')
        ...
        with torch.no_grad():
            with tqdm(total=len(dev_loader)) as pbar:
                for i, (x, y, wn_ids) in enumerate(dev_loader):
                    ...
                    pbar.update(1)
                    result, indices = torch.topk(out, k=100, dim=-1,
...
def getPredFromDesc(model, desc: str, mask_size=5, top_n=10):
    ...
    cls_id, mask_id, = train_dataset.cls_id, train_dataset.mask_id
    ...
    out = model(input_ids=x, attention_mask=attention_mask,
                target_matrix=target_matrix)
    result, indices = torch.topk(out, k=top_n, dim=-1,
                                 largest=True, sorted=True)

    indices = indices[0]
    return [idx2target[i] for i in indices], \
        indices, torch.sigmoid(result).squeeze(0)
